async function applyCode(promoCode, utils) {
  const requestUrl = $('form.promo-code-form').attr('action') || '/us/applycoupon';
...
async function removeCode(promoCode, uuid, utils) {
  const requestUrl =
    $('button.delete-coupon-confirmation-btn').attr('data-action') || '/on/demandware.store/Sites-ganni_us-Site/en_US/Cart-RemoveCouponLineItem';
