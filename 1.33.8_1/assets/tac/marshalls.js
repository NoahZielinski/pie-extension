async function applyCodeCart(promoCode, utils) {
  ...
  try {
    const response = await $.ajax({
      url: 'https://www.marshalls.com/us/store/checkout/cart.jsp?_DARGS=/us/store/checkout/views/cart.jsp.promo',
      type: 'POST',
      data: serializedForm,
...
async function applyCodeCheckout(promoCode, utils) {
  ...
  try {
    const response = await $.ajax({
      url: 'https://www.marshalls.com/us/store/checkout/checkout.jsp?shipping=true&_DARGS=/us/store/checkout/includes/longScrollPromoBoxPanel.jsp.checkoutPromoForm',
      type: 'POST',
      data: serializedForm,
      contentType: 'application/x-www-form-urlencoded',
      headers: {
...
async function updatePrice(cartPriceSelector, originalPrice, utils, couponLocation) {
  const requestUrl =
    couponLocation === 'cart' ? 'https://www.marshalls.com/us/store/checkout/cart.jsp' : 'https://www.marshalls.com/us/store/checkout/checkout.jsp';
  let newPrice;
  ...
  try {
    ...
    newPrice = $(updatePriceResponse.responseText).find(cartPriceSelector).text();
  } catch (error) {
...
async function tacSubmit({ utils, promoCode, runState, cartPriceSelector, currentPrice, isApplyBest }) {
  const CHECKOUT_URL_REGEX = /\/store\/checkout\/checkout\.jsp/;
  ...
  finalPrice = await updatePrice(cartPriceSelector, originalPrice, utils, couponLocation);
