async function applyCode(promoCode, utils) {
  const requestUrl =
    ($('script')
      .text()
      .match(/"addCoupon":"([^\\"]+)?"/) || [])[1] || 'https://www.boohooman.com/on/demandware.store/Sites-boohooman-EU-Site/de_DE/Cart-AddCouponJson';
...
async function removeCode(promoCode, utils) {
  const requestUrl =
    ($('script')
      .text()
      .match(/"removeCoupon":"([^\\"]+)?"/) || [])[1] || 'https://www.boohooman.com/on/demandware.store/Sites-boohooman-EU-Site/de_DE/COBilling-RemoveCoupon';
  const dtpc = (document.cookie.match(/dtPC=([^;]+)?/) || [])[1] || '';
