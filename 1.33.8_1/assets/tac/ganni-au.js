async function applyCode(promoCode, utils) {
  const requestUrl = $('form.promo-code-form').attr('action') || '/en-au/applycoupon';
...
async function removeCode(promoCode, uuid, utils) {
  const requestUrl = $('button.delete-coupon-confirmation-btn').attr('data-action') || '/on/demandware.store/Sites-ganni-Site/en_AU/Cart-RemoveCouponLineItem';
