async function applyCode(promoCode, utils) {
  const requestUrl = $('form[name="promo-code-form"]').attr('action') || '/on/demandware.store/Sites-boohoo-EU-Site/fr_FR/Cart-AddCoupon?source=checkout';
...
async function removeCode(promoCode, uuid, utils) {
  const requestUrl =
    ...
    '/on/demandware.store/Sites-boohoo-EU-Site/fr_FR/Cart-RemoveCouponLineItem?source=checkout';
  const token = $('[data-token-name="csrf_token"]').attr('data-token-value') || '';
