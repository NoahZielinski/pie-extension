(function() {
                    ...
                    return new Promise(function(resolve, reject) {
                        const processAfter = async function(response) {
                            if (response.status == 200) {
                                ...
                                const cachedM3u8Url = streamInfo != null && streamInfo.EncodingsM3U8 != null && streamInfo.EncodingsM3U8.match(/^https:.*\.m3u8$/m)?.[0];
                                if (cachedM3u8Url && (await realFetch(cachedM3u8Url)).status !== 200) {
    ...
    function stripAdSegments(textStr, stripAllSegments, streamInfo) {
        ...
        if (hasStrippedAdSegments) {
            for (let i = 0; i < lines.length; i++) {
                ...
                // Strip ad metadata tags (EXT-X-DATERANGE, X-TV-TWITCH-AD-*, etc.) so the
                // player's native "Ad (0:xx)" countdown text doesn't show even when we're
                // successfully stripping the ad video segments. Since we don't use the
                // DOM overlay to cover this text, we have to strip the underlying tags.
                if (AdSignifiers.some((s) => lines[i].includes(s))) {
                    lines[i] = '';
                }
            }
        } else {
    ...
    function monitorPlayerBuffering() {
        ...
        // Track hasHadData unconditionally so the MSE-starved watchdog below can fire even
        // when isActivelyStrippingAds hasn't toggled on this tick.
        if (playerForMonitoringBuffering) {
            try {
                const video = playerForMonitoringBuffering.player?.getHTMLVideoElement?.();
                if (video && video.readyState >= 3) {
                    playerBufferState.hasHadData = true;
                }
            } catch {}
        }
        // MSE-starved watchdog: catches freezes where the video stays paused with rs<3 for
        // a sustained period, even when isActivelyStrippingAds has toggled back to false
        // (e.g. during backup-player-type cycling, the flag flips on/off rapidly and the
        // ad-strip stall check below never accumulates). A paused+rs<3 state means MSE
        // has no data ahead of playhead — the player cannot recover without rebuilding
        // the MediaSource via setSrc. Gated only on hasHadData (to avoid init false-
        // positives) and userPauseIntent (respect user's pause). 8s threshold is
        // conservative to avoid tripping on brief buffering dips.
        if (playerForMonitoringBuffering && playerBufferState.hasHadData && !playerBufferState.userPauseIntent) {
            try {
                const video = playerForMonitoringBuffering.player?.getHTMLVideoElement?.();
                if (video && !video.ended) {
                    const mseStarved = video.paused && video.readyState < 3;
                    const mseStallCooldown = 15000;
                    const mseCooldownExpired = !playerBufferState.lastMseStallReloadAt || (Date.now() - playerBufferState.lastMseStallReloadAt) > mseStallCooldown;
                    if (mseStarved && mseCooldownExpired) {
                        if (!playerBufferState.mseStallStartAt) {
                            playerBufferState.mseStallStartAt = Date.now();
                        } else if ((Date.now() - playerBufferState.mseStallStartAt) > 8000) {
                            console.log('[AD DEBUG] MSE starved — video paused with rs=' + video.readyState + ' for ' + ((Date.now() - playerBufferState.mseStallStartAt) / 1000).toFixed(1) + 's — hard reload');
                            playerBufferState.lastMseStallReloadAt = Date.now();
                            playerBufferState.mseStallStartAt = 0;
    ...
    function updateAdblockBanner(data) {
        ...
        if (data.hasAds) {
            document.body?.dispatchEvent(new Event('twitch-show-adblock-banner'));
            hideTwitchAdOverlays();
        } else {
            document.body?.dispatchEvent(new Event('twitch-hide-adblock-banner'));
        }
    }
    ...
    function hookFetch() {
        ...
        window.fetch = maskAsNative(function(url, init, ...args) {
            if (typeof url === 'string') {
                if (url.includes('gql') && init && typeof init.headers === 'object' && init.headers) {
                    ...
                    // Get rid of mini player above chat — intercept response to null out PBP entries
                    let pbpIndices = null;
                    if (typeof init.body === 'string' && init.body.includes('PlaybackAccessToken') && init.body.includes('picture-by-picture')) {
                        try {
                            const pbpBody = JSON.parse(init.body);
                            if (Array.isArray(pbpBody)) {
                                pbpIndices = [];
                                for (let i = 0; i < pbpBody.length; i++) {
                                    if (pbpBody[i]?.operationName === 'PlaybackAccessToken_Template' && pbpBody[i]?.variables?.playerType === 'picture-by-picture') {
                                        pbpIndices.push(i);
                                    }
                                }
                                if (pbpIndices.length === 0) pbpIndices = null;
                                else if (pbpIndices.length === pbpBody.length) {
                                    return Promise.resolve(new Response(
                                        JSON.stringify(pbpBody.map(() => ({ data: { streamPlaybackAccessToken: null } }))),
                                        { status: 200, headers: { 'Content-Type': 'application/json' } }
                                    ));
                                }
                            } else if (pbpBody?.variables?.playerType === 'picture-by-picture') {
                                return Promise.resolve(new Response(
                                    JSON.stringify({ data: { streamPlaybackAccessToken: null } }),
                                    { status: 200, headers: { 'Content-Type': 'application/json' } }
                                ));
                            }
                        } catch(e) {}
                    ...
                    if (ForceAccessTokenPlayerType && typeof init.body === 'string' && init.body.includes('PlaybackAccessToken')) {
                        ...
                        if (Array.isArray(newBody)) {
                            for (let i = 0; i < newBody.length; i++) {
                                if (newBody[i]?.operationName?.includes('PlaybackAccessToken') && newBody[i]?.variables?.playerType && newBody[i]?.variables?.playerType !== ForceAccessTokenPlayerType && newBody[i]?.variables?.playerType !== 'frontpage') {
                                    replacedPlayerType = newBody[i].variables.playerType;
                                    newBody[i].variables.playerType = ForceAccessTokenPlayerType;
                                }
                            }
                        } else {
                            if (newBody?.operationName?.includes('PlaybackAccessToken') && newBody?.variables?.playerType && newBody?.variables?.playerType !== ForceAccessTokenPlayerType && newBody?.variables?.playerType !== 'frontpage') {
                    ...
                    if (pbpIndices) {
                        return realFetch.apply(this, arguments).then(async (response) => {
                            const cloned = response.clone();
                            try {
                                const responseBody = await cloned.json();
                                if (Array.isArray(responseBody)) {
                                    for (const idx of pbpIndices) {
                                        if (idx < responseBody.length) {
                                            responseBody[idx] = { data: { streamPlaybackAccessToken: null } };
                                        }
                                    }
                                }
    ...
    function onContentLoaded() {
        ...
        // CSS to hide Turbo/upsell banners that flash during ad blocking
        try {
            const adUpsellStyle = document.createElement('style');
            adUpsellStyle.textContent = '.player-overlay-background:has(a[href*="turbo"]) { display: none !important; } .player-overlay-background:has(a[href*="how-to-allow-ads"]) { display: none !important; } .stream-display-ad__container_squeezeback { display: none !important; } .stream-display-ad__container_lower-third { display: none !important; } .turbo-sda-upsell { display: none !important; } [role="banner"][aria-label*="Turbo"] { display: none !important; } .channel-info-content > div > div:has([role="banner"][aria-label*="Turbo"]) { display: none !important; }';
            (document.head || document.documentElement).appendChild(adUpsellStyle);
        } catch {}
        // Fix channel info alignment when transitioning from offline to live
        try {
            const fixChannelInfoAlignment = () => {
                const channelRoot = document.querySelector('.channel-root');
                if (!channelRoot) return;
                const observer = new ResizeObserver(() => {
                    const infoContent = document.querySelector('.channel-info-content');
                    if (infoContent) { infoContent.style.maxWidth = ''; void infoContent.offsetHeight; }
                });
                ...
                const mutationObs = new MutationObserver(() => {
                    const videoContainer = document.querySelector('.video-player__container');
                    if (videoContainer) { observer.disconnect(); observer.observe(videoContainer); }
                });
                mutationObs.observe(channelRoot, { childList: true, subtree: true });
            };
            fixChannelInfoAlignment();
        } catch {}
    }
    ...
    try { hookWindowWorker(); } catch (err) { console.error('[AD DEBUG] hookWindowWorker failed: ' + err); }
    hookFetch();
