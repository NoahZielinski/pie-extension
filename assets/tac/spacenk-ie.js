async function applyCode(promoCode, utils) {
  const requestUrl = $('form[name="promo-code-form"]').attr('action') || '/on/demandware.store/Sites-spacenkgb-Site/en_IE/Cart-AddCoupon';
  const token = $('form[name="promo-code-form"] input[name="csrf_token"]').val() || '';
...
async function removeCode(promoCode, uuid, utils) {
  const requestUrl =
    $('button.delete-coupon-confirmation-btn').attr('data-action') || '/on/demandware.store/Sites-spacenkgb-Site/en_IE/Cart-RemoveCouponLineItem';
...
function updatePrice(applyCodeResponse, cartPriceSelector, originalPrice, utils) {
  ...
  try {
    const priceString = applyCodeResponse?.totals?.grandTotal || originalPrice;
